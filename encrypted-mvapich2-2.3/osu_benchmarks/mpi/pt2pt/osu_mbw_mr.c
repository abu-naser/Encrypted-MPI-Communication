...
#define MY_DATA_SIZE 10
//int my_datasize[100] = {  32*1024, 64*1024, 128*1024, 256*1024, 512*1024, 1024*1024, 2*1024*1024}; // 7
//int my_datasize[100] = {  32*1024, 64*1024, 128*1024, 256*1024, 512*1024}; // 5
//int my_datasize[100] = {  32*1024, 64*1024, 128*1024, 256*1024, 512*1024, 1024*1024}; // 6
//int my_datasize[100] = { 32*1024, 64*1024, 128*1024, 256*1024}; // 4
//int my_datasize[100] = {256};
int my_datasize[100] = {1, 16, 256, 1024, 4*1024, 16*1024, 64*1024, 256*1024, 1024*1024, 2*1024*1024}; //10
int ite;
/* end of add */
...
int main(int argc, char *argv[])
{
   ...
   if(options.window_varied) {
       ...
       mbw_reqstat = (MPI_Status *)malloc(sizeof(MPI_Status) * options.window_size);
        ...
        printf( "Calling Multipair for the cluster-2019\n .............................\n");
       //for(curr_size = options.min_message_size; curr_size <= options.max_message_size; curr_size *= 2) {
         for(ite = 0; ite < MY_DATA_SIZE; ite++) {
            curr_size = my_datasize[ite];
           double bw, rate;
    ...
    if(rank < num_pairs) {
        ...
        for(i = 0; i <  options.iterations +  options.skip; i++) {
            ...
            for(j = 0; j < window_size; j++) {
                MPI_CHECK(MPI_SEC_Isend(s_buf, size, MPI_CHAR, target, 100, MPI_COMM_WORLD,
                        mbw_request + j));
            }
            MPI_CHECK(MPI_SEC_Waitall(window_size, mbw_request, mbw_reqstat));
        ...
        for(i = 0; i <  options.iterations +  options.skip; i++) {
            ...
            for(j = 0; j < window_size; j++) {
                MPI_CHECK(MPI_SEC_Irecv(r_buf, size, MPI_CHAR, target, 100, MPI_COMM_WORLD,
                        mbw_request + j));
            }
