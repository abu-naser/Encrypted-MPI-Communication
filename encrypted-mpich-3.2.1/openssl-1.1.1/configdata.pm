...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/home/gavahi/openssl-1.1.1/openssl_install no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.16.3",
  perlargv => [ "linux-x86_64", "--prefix=/home/gavahi/openssl-1.1.1/openssl_install" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
  ...
  prefix => "/home/gavahi/openssl-1.1.1/openssl_install",
  processor => "",
